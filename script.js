...
function updateProgressVisualization() {
  ...
  if (progressBars.length === 3) {
    const todoProgress = calculateCategoryProgress("todo");
    const inProgressProgress = calculateCategoryProgress("in-progress");
    const doneProgress = calculateCategoryProgress("done");

    progressBars[0].style.width = `${todoProgress}%`;
    progressBars[1].style.width = `${inProgressProgress}%`;
    progressBars[2].style.width = `${doneProgress}%`;

    progressTexts[0].textContent = `${todoProgress}%`;
    progressTexts[1].textContent = `${inProgressProgress}%`;
    progressTexts[2].textContent = `${doneProgress}%`;
  }
}
