...
function updateUIForLoggedInUser(user) {
  ...
  // Add event listener to the dropdown logout button (using the unique ID)
  document
...
function handleSignup(e) {
  ...
  // Create user first, then check/save username
  auth
    ...
    .catch((error) => {
      ...
      if (error.code === "auth/email-already-in-use") {
        ...
        showAuthMessage("Invalid email address format.", "error");
      } else {
        showAuthMessage(error.message, "error");
      }
    });
