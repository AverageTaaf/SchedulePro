...
function createTaskElement(task) {
  ...
  ${
    task.status !== "archived"
      ...
      : `<button class="btn-success unarchive-task"><i class="fas fa-box-open"></i>Unarchive</button>`
  }
  </div>
  `;
  ...
  // Add unarchive listener for archived tasks
  if (task.status === "archived") {
    taskElement
      .querySelector(".unarchive-task")
...
function updateProgressVisualization() {
  ...
  // Calculate percentages based on total tasks, not average progress
  ...
  if (progressBars.length >= 3) {
    progressBars[0].style.width = `${todoPercentage}%`;
    progressBars[1].style.width = `${inProgressPercentage}%`;
    progressBars[2].style.width = `${donePercentage}%`;
  }
  ...
  if (progressTexts.length >= 3) {
    progressTexts[0].textContent = `${todoPercentage}%`;
    progressTexts[1].textContent = `${inProgressPercentage}%`;
    progressTexts[2].textContent = `${donePercentage}%`;
  }
}
